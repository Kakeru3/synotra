...
impl Interpreter {
    pub fn new(program: Program) -> Self {
        let mut builtin_functions: HashMap<String, Arc<dyn Fn(Vec<Value>) -> Value + Send + Sync>> = HashMap::new();
    ...
    pub fn run(&self) -> Result<(), String> {
        // main タスクを探す
        let main_task = self.program.find_task("main")
    ...
    fn execute_task(&self, task: &Task, args: Vec<Value>) -> Result<TaskResult, String> {
        eprintln!("DEBUG: Executing task: {}", task.name);
        ...
        // タスクの本体を実行
        let result = self.execute_statements(&task.body, &mut env);
        eprintln!("DEBUG: Task {} finished with: {:?}", task.name, result);
        result
    }

    /// 文のリストを実行
    fn execute_statements(&self, statements: &[Statement], env: &mut Environment) -> Result<Value, String> {
    ...
    /// 文を実行
    fn execute_statement(&self, stmt: &Statement, env: &mut Environment) -> Result<Value, ControlFlow> {
        match stmt {
            ...
            Statement::FunctionDef(func) => {
                eprintln!("DEBUG: Defining function: {}", func.name);
            ...
            Statement::If { condition, then_branch, else_branch } => {
                ...
                if self.is_truthy(&cond_val) {
                    self.execute_statements(then_branch, env).map_err(ControlFlow::Error)
                } else {
                    self.execute_statements(else_branch, env).map_err(ControlFlow::Error)
                }
            }
            ...
            Statement::For { var, start, end, body } => {
                ...
                eprintln!("DEBUG: For loop {} from {:?} to {:?}", var, start_val, end_val);
                
                let start_num = if let Value::Int(n) = start_val {
                    n
                } else {
                    return Err(ControlFlow::Error("For loop start must be an integer".to_string()));
                };
                
                let end_num = if let Value::Int(n) = end_val {
                    n
                } else {
        ...
        match expr {
            ...
            Expression::Var(name) => {
                env.get(name).ok_or_else(|| {
                    ControlFlow::Error(format!("Undefined variable: {}", name))
                })
            }
    ...
    fn eval_binary_op(&self, op: &str, left: Value, right: Value) -> Result<Value, ControlFlow> {
        match (op, left, right) {
            ...
            ("==", Value::Int(l), Value::Int(r)) | ("=", Value::Int(l), Value::Int(r)) => Ok(Value::Bool(l == r)),
            ("==", Value::String(l), Value::String(r)) | ("=", Value::String(l), Value::String(r)) => Ok(Value::Bool(l == r)),
            ("==", Value::Bool(l), Value::Bool(r)) | ("=", Value::Bool(l), Value::Bool(r)) => Ok(Value::Bool(l == r)),
            // not-equal
    ...
    /// 関数を呼び出す
    fn call_function(&self, name: &str, args: Vec<Value>, env: &Environment) -> Result<Value, ControlFlow> {
        // 組み込み関数をチェック
        ...
        if let Some(func) = env.get_function(name) {
            ...
            return self.execute_statements(&func.body, &mut new_env).map_err(ControlFlow::Error);
        }
